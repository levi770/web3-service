...
export class ExportService {
  ...
  async wrapInZip(password: string, data: string, name: string) {
    let archive = (archiver as any).create('zip-encrypted', { zlib: { level: 8 }, encryptionMethod: 'aes256', password });
  ...
  async exportAccounts(type: ExportTypes) {
    switch (type) {
      ...
      case ExportTypes.TOKEN: {
        ...
        return (
          'id,token_ids,wallet_id,contract_id,metadata_id,status,qty,created_at\n' +
          rows
            .map((e: TokenModel) => {
              return [e.id, e.token_ids, e.wallet_id, e.contract_id, e.metadata_id, e.status, e.qty, e.createdAt];
            })
            .map((e) => e.join(','))
            .join('\n')
        );
      ...
      case ExportTypes.METADATA: {
        const { rows } = await this.repository.getAllObjects<MetadataModel>(ObjectTypes.TRANSACTION);
        return (
          'id,contract_id,token_id,slug,status,type,meta_data,created_at\n' +
          rows
            .map((e: MetadataModel) => {
              return [e.id, e.contract_id, e.token_id, e.slug, e.status, e.type, e.meta_data, e.createdAt];
            })
            .map((e) => e.join(','))
            .join('\n')
        );
      ...
      case ExportTypes.WHITELIST: {
        const { rows } = await this.repository.getAllObjects<WhitelistModel>(ObjectTypes.TRANSACTION);
        return (
