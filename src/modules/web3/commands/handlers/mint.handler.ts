...
export class MintHandler implements ICommandHandler<MintCommand> {
  ...
  async execute(command: MintCommand) {
    try {
      ...
      const [tokenModel] = await this.dbService.create<TokenModel>([tokenPayload], ObjectTypes.TOKEN);
      ...
      return new ResponseDto(HttpStatus.OK, [Statuses.SUCCESS], [{ tx, token: tokenModel }]);
    } catch (error) {
