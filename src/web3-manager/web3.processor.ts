...
export class Web3Processor {
  ...
  async processWhitelist(job: Job): Promise<CallResultDto> {
    try {
      ...
      // Get the contract object and the whitelist options from the call data
      ...
      // Create a transaction object to be used in the `send` function
      const txObj: TxObj = {
      ...
      // Send the transaction using the `send` function
  ...
  async processCall(job: Job): Promise<CallResultDto> {
    try {
      ...
      // If the contract is not found, throw an error
      if (!contractObj) {
        throw new RpcException('contract not found');
      }
      ...
      const contractInst = new w3.eth.Contract(contractObj.deploy_data.abi as U.AbiItem[], contractObj.address);
      // Find the ABI object for the specified method
      ...
      switch (callData.operation_type) {
        ...
        case OperationTypes.MINT:
          ...
          // Get the mint options from the operation specific options. Throw an error if the options are not present.
          const mintOptions = callData?.operation_options as MintDataDto;
          if (!mintOptions) {
            throw new RpcException('operation specific options missed');
          }
          // If the execute flag is set, set the status to PROCESSED. Otherwise, set it to CREATED.
          ...
          // Create a new token object with the obtained data and store it in the database.
          ...
          if (mintOptions.meta_data && mintOptions.asset_url && mintOptions.asset_type) {
            ...
            // Create a new metadata object with the obtained data and store it in the database.
            ...
            // Set the metadata for the created token object in the database.
          ...
          // Set the metadata for the created token object to the metadata of the contract object, stored in the database.
          await this.dbManager.setMetadata(
            { object_id: tokenObj[0].id, metadata_id: metadataObj[0].id },
            ObjectTypes.TOKEN,
          );
  ...
  async deploy(job: Job): Promise<DeployResultDto> {
    try {
      ...
      // Create a transaction object to be used in the send function, including the encoded ABI data.
      const txObj: TxObj = {
      ...
      // If metadata is provided, create a metadata object with the metadata and store it in the database.
      if (deployData.meta_data && deployData.asset_url && deployData.asset_type) {
        ...
        // Link the metadata object to the contract object in the database.
        await this.dbManager.setMetadata(
  ...
  async getArgs(args: string, inputs: U.AbiInput[]): Promise<any[]> {
    try {
      ...
      // If the length of the arguments array is not the same as the length of the ABI input objects, throw an exception.
      if (argsArr.length !== inputs.length) {
        throw new RpcException('arguments length is not valid');
      }
      // Map the arguments array to the corresponding ABI input objects and return the resulting array.
